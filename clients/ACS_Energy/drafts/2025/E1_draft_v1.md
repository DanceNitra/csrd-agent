# E1 — Climate Change

**Client:** ACS_Energy
**Report Year:** 2025
**Date:** 2026-06-08
**Draft Version:** v1

---

## Materiality Context

This standard is material for ACS_Energy. 
Impact score: 3.0, Financial score: 1.5

## E1-1 — Transition plan for climate change mitigation

### E1-1_01 **(Mandatory)**

*Whether company has a transition plan for climate mitigation*

[DATA PENDING — to be filled from data collection phase]

### E1-1_02 

*GHG emission reduction targets and deadlines*

[DATA PENDING — to be filled from data collection phase]

### E1-1_03 

*Decarbonization levers identified*

[DATA PENDING — to be filled from data collection phase]

### E1-1_04 

*Investment and funding plan for transition*

[DATA PENDING — to be filled from data collection phase]

### E1-1_05 

*Alignment with EU Taxonomy*

[DATA PENDING — to be filled from data collection phase]

## E1-2 — Policies related to climate change mitigation and adaptation

### E1-2_01 

*Climate change mitigation policies*

[DATA PENDING — to be filled from data collection phase]

### E1-2_02 

*Climate change adaptation policies*

[DATA PENDING — to be filled from data collection phase]

### E1-2_03 

*Scope of policies (own ops, value chain)*

[DATA PENDING — to be filled from data collection phase]

## E1-3 — Actions and resources related to climate change

### E1-3_01 

*Key actions for climate mitigation and adaptation*

[DATA PENDING — to be filled from data collection phase]

### E1-3_02 

*Resources allocated (CapEx, OpEx) for climate actions*

[DATA PENDING — to be filled from data collection phase]

## E1-4 — Targets related to climate change mitigation and adaptation

### E1-4_01 

*GHG reduction targets (absolute and intensity)*

[DATA PENDING — to be filled from data collection phase]

### E1-4_02 

*Base year and target year*

[DATA PENDING — to be filled from data collection phase]

### E1-4_03 

*Progress against targets*

[DATA PENDING — to be filled from data collection phase]

### E1-4_04 

*Whether targets are science-based (SBTi)*

[DATA PENDING — to be filled from data collection phase]

## E1-5 — Energy consumption and mix

### E1-5_01 

*Total energy consumption (MWh)*

[DATA PENDING — to be filled from data collection phase]

### E1-5_02 

*Fossil fuel consumption (MWh)*

[DATA PENDING — to be filled from data collection phase]

### E1-5_03 

*Nuclear energy consumption (MWh)*

[DATA PENDING — to be filled from data collection phase]

### E1-5_04 

*Renewable energy consumption (MWh)*

[DATA PENDING — to be filled from data collection phase]

### E1-5_05 

*Renewable energy share in total consumption (%)*

[DATA PENDING — to be filled from data collection phase]

### E1-5_06 

*Energy intensity (MWh per net revenue)*

[DATA PENDING — to be filled from data collection phase]

## E1-6 — Gross Scopes 1, 2, 3 and Total GHG emissions

### E1-6_01 

*Gross Scope 1 GHG emissions (tCO2e)*

[DATA PENDING — to be filled from data collection phase]

### E1-6_02 

*Gross Scope 2 location-based GHG emissions (tCO2e)*

[DATA PENDING — to be filled from data collection phase]

### E1-6_03 

*Gross Scope 2 market-based GHG emissions (tCO2e)*

[DATA PENDING — to be filled from data collection phase]

### E1-6_04 

*Gross Scope 3 GHG emissions — total (tCO2e)*

[DATA PENDING — to be filled from data collection phase]

### E1-6_05 

*Scope 3 Category 1 — Purchased goods and services*

[DATA PENDING — to be filled from data collection phase]

### E1-6_06 

*Scope 3 Category 2 — Capital goods*

[DATA PENDING — to be filled from data collection phase]

### E1-6_07 

*Scope 3 Category 3 — Fuel and energy-related activities*

[DATA PENDING — to be filled from data collection phase]

### E1-6_08 

*Scope 3 Category 4 — Upstream transportation and distribution*

[DATA PENDING — to be filled from data collection phase]

### E1-6_09 

*Scope 3 Category 5 — Waste generated in operations*

[DATA PENDING — to be filled from data collection phase]

### E1-6_10 

*Scope 3 Category 6 — Business travel*

[DATA PENDING — to be filled from data collection phase]

### E1-6_11 

*Scope 3 Category 7 — Employee commuting*

[DATA PENDING — to be filled from data collection phase]

### E1-6_12 

*Scope 3 Category 8 — Upstream leased assets*

[DATA PENDING — to be filled from data collection phase]

### E1-6_13 

*Scope 3 Category 9 — Downstream transportation*

[DATA PENDING — to be filled from data collection phase]

### E1-6_14 

*Scope 3 Category 10 — Processing of sold products*

[DATA PENDING — to be filled from data collection phase]

### E1-6_15 

*Scope 3 Category 11 — Use of sold products*

[DATA PENDING — to be filled from data collection phase]

### E1-6_16 

*Scope 3 Category 12 — End-of-life treatment of sold products*

[DATA PENDING — to be filled from data collection phase]

### E1-6_17 

*Scope 3 Category 13 — Downstream leased assets*

[DATA PENDING — to be filled from data collection phase]

### E1-6_18 

*Scope 3 Category 14 — Franchises*

[DATA PENDING — to be filled from data collection phase]

### E1-6_19 

*Scope 3 Category 15 — Investments*

[DATA PENDING — to be filled from data collection phase]

### E1-6_20 

*Total GHG emissions (Scope 1 + 2 + 3)*

[DATA PENDING — to be filled from data collection phase]

### E1-6_21 

*GHG intensity (tCO2e per net revenue)*

[DATA PENDING — to be filled from data collection phase]

## E1-7 — GHG removals and GHG mitigation projects financed through carbon credits

### E1-7_01 

*GHG removals (tCO2e) from own operations*

[DATA PENDING — to be filled from data collection phase]

### E1-7_02 

*Carbon credits purchased and retired (tCO2e)*

[DATA PENDING — to be filled from data collection phase]

### E1-7_03 

*Carbon credit certification standards used*

[DATA PENDING — to be filled from data collection phase]

## E1-8 — Internal carbon pricing

### E1-8_01 

*Whether internal carbon pricing is applied*

[DATA PENDING — to be filled from data collection phase]

### E1-8_02 

*Type of internal carbon pricing (shadow price, internal tax, internal fund)*

[DATA PENDING — to be filled from data collection phase]

### E1-8_03 

*Price applied (€/tCO2e)*

[DATA PENDING — to be filled from data collection phase]

## E1-9 — Anticipated financial effects from material physical and transition risks

### E1-9_01 

*Financial effects from physical climate risks*

[DATA PENDING — to be filled from data collection phase]

### E1-9_02 

*Financial effects from transition risks*

[DATA PENDING — to be filled from data collection phase]

### E1-9_03 

*Climate-related opportunities*

[DATA PENDING — to be filled from data collection phase]

---
*Auto-generated by CSRD Report Engine | 2026-06-08 15:49*